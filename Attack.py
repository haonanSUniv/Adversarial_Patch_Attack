...
parser.add_argument('--train_size', type=int, default=5, help="number of training images")
parser.add_argument('--test_size', type=int, default=5, help="number of test images")
...
def patch_attack(image, applied_patch, mask, target, probability_threshold, model, lr=1, max_iteration=100):
    ...
    perturb = perturbated_image[0]
    plt.imshow(np.clip(np.transpose(perturb,(1,2,0)),0,1))
    plt.savefig("/content/Adversarial_Patch_Attack/experiment_statistics/" + str(epoch) + " patched.png")
...
# Load the datasets
train_loader, test_loader = dataloader(args.train_size, args.test_size, args.data_dir, args.batch_size, args.num_workers, 10)
...
for epoch in range(args.epochs):
    ...
    for (image, label) in train_loader:
        ...
        if predicted[0] != label and predicted[0].data.cpu().numpy() != args.target:
             ...
             print("img shape",(image.size(1),image.size(2),image.size(3)))
             applied_patch, mask, x_location, y_location = mask_generation(args.patch_type, patch, image_size=(image.size(1),image.size(2),image.size(3)))
    ...
    plt.savefig("/content/Adversarial_Patch_Attack/experiment_statistics/3.5%noise/pictures/" + str(epoch) + " patch.png")
    ...
    if test_success_rate > best_patch_success_rate:
        ...
        plt.imshow(np.clip(np.transpose(patch, (1, 2, 0)) * std + mean, 0, 1))
        plt.savefig("/content/Adversarial_Patch_Attack/experiment_statistics/best_patch.png")
